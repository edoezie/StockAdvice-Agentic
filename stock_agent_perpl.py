...
# Initialize the local Ollama endpoint OR Perplexity API
def setup_llm():
    # return OllamaLLM(
...
def news_analysis(state: InputState) -> Dict:
    ...
    if analysis:
        print("Results found from LLM in News: \n", analysis.content)
    else:
        print ("News LLM chain not functional - EXITING!")
        exit()
...
print ("\nRESULTS: ", result["results"]["recommendation"].content)
